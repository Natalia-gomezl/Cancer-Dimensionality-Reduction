...
* **`Datos tumores`**: Contiene los archivos `data.csv` y `labels.csv` con la información genómica.
* **`Método PCA`**: Incluye información sobre el método, los scripts y las gráficas que resultan
*  **`Método T-SNE`**: Incluye información sobre el método, los scripts y las gráficas que resultan
*  En estas  dos carpetas se encontrarán subcarpetas organizadas de la siguiente manera: 
* **`Scripts/`**: Incluye los códigos fuente ejecutables en R.
* **`Descripción del  método/`**: Documentación técnica detallada de cada metodología aplicada.
